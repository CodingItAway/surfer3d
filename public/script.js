...
// import { GLTFLoader } from 'three/examples/jsm/loaders/GLTFLoader.js'; // Uncomment when using models
...
function init() {
  ...
  // Player (Using Capsule for now, ready for GLTF)
...
// Function to translate touches into game actions
function handleSwipe(startX, startY, endX, endY) {
  const deltaX = endX - startX;
  const deltaY = endY - startY;
  const minSwipeDistance = 30; // Must swipe at least 30px

  if (Math.abs(deltaX) < minSwipeDistance && Math.abs(deltaY) < minSwipeDistance) return; // Ignore taps
  ...
  if (Math.abs(deltaX) > Math.abs(deltaY)) {
    ...
    if (deltaY < 0 && player.position.y <= 1.8) {
      ...
      // Swipe Down (Slide)
      slideTimer = 35; // Slide lasts for ~35 frames
    }
  }
}
...
function animate() {
  ...
  // Ground Control (Keyboard & Touch Slide logic combined)
